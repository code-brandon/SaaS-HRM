...
@Api(tags = "考勤配置表")
@RestController
@RequestMapping("attendance/atteattendanceconfig")
public class AtteAttendanceConfigController {
    ...
    @GetMapping("/info/{id}")
    public R<AtteAttendanceConfigEntity> info(@PathVariable("id") String id){
            AtteAttendanceConfigEntity atteAttendanceConfig = atteAttendanceConfigService.getById(id);

        return Objects.nonNull(atteAttendanceConfig) ? R.ok("查询成功").data(atteAttendanceConfig) : R.error("查询失败");
    }
    ...
    public R<Boolean> save(@RequestBody @ApiParam(name="考勤配置表",value="考勤配置表 实体对象",required=true) AtteAttendanceConfigEntity atteAttendanceConfig){
        ...
        return atteAttendanceConfigService.save(atteAttendanceConfig) ? R.ok("保存成功").data(true) : R.error("保存失败").data(false);
    }
    ...
    public R<Boolean> update(@RequestBody @ApiParam(name="考勤配置表",value="考勤配置表 实体对象",required=true) AtteAttendanceConfigEntity atteAttendanceConfig){
        ...
        return atteAttendanceConfigService.updateById(atteAttendanceConfig) ? R.ok("修改成功").data(true) : R.error("修改失败").data(false);
    }
    ...
    public R<Boolean> delete(@RequestBody @ApiParam(name="ID",value="ID集合",required=true) String[] ids){
        ...
        return atteAttendanceConfigService.removeByIds(Arrays.asList(ids)) ? R.ok("删除成功").data(true) : R.error("删除失败").data(false);
    }
