...
public class EmPositiveController {
    ...
    public R<Boolean> saveOrUpdate(@RequestBody @ApiParam(name="员工岗位信息表",value=" 实体对象",required=true) EmPositiveEntity emPositive){
        return emPositiveService.updateOrSave(emPositive) ? R.ok("保存成功").data(true) : R.error(ResultCode.FAIL.code(),"保存失败").data(false);
    }
