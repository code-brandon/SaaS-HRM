...
public class EmUserCompanyController {
    ...
    public R<EmUserCompanyEntity> info(@PathVariable("userId") String userId){
        ...
        return Objects.nonNull(emUserCompany) ? R.ok("查询成功").data(emUserCompany) : R.error(ResultCode.FAIL.code(),"查询失败");
    }
    ...
    public R<Boolean> save(@RequestBody @ApiParam(name="员工公司表",value=" 实体对象",required=true) EmUserCompanyEntity emUserCompany){
        ...
        return emUserCompanyService.save(emUserCompany) ? R.ok("保存成功").data(true) : R.error(ResultCode.FAIL.code(),"保存失败").data(false);
    }
    ...
    public R<Boolean> update(@RequestBody @ApiParam(name="员工公司表",value=" 实体对象",required=true) EmUserCompanyEntity emUserCompany){
        ...
        return emUserCompanyService.updateById(emUserCompany) ? R.ok("修改成功").data(true) : R.error(ResultCode.FAIL.code(),"修改失败").data(false);
    }
    ...
    public R<Boolean> delete(@RequestBody @ApiParam(name="ID",value="ID集合",required=true) String[] userIds){
        ...
        return emUserCompanyService.removeByIds(Arrays.asList(userIds)) ? R.ok("删除成功").data(true) : R.error(ResultCode.FAIL.code(),"删除失败").data(false);
    }
